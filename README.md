...
* `/stop`：强制中断。当后台正在执行耗时命令或代码生成时，发送此指令可立即终止后台对应的模型子进程并释放资源。

### 五、 并发与状态管理
1. **用户级进程并发锁**：为防止服务器资源枯竭，项目实现了并发保护。当一个任务正在深度执行时，若用户连续发送新消息或狂点按钮，系统会自动拦截并提示排队，直到当前任务完成或被 `/stop` 终止。
2. **SQLite 数据持久化**：所有的长时偏好记忆和对话上下文均存储在原生的 SQLite 数据库中，保证了并发读写的数据安全性。
